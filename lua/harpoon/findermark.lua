...
local function get_dir_from_name(filename)
    if vim.fn.isdirectory(filename) == 1 then
        return filename
    end

    for idx = #filename, 1, -1 do
        local char = string.sub(filename, idx, idx)
        if char == "/" then
            local dir = string.sub(filename, 1, idx - 1)
            return dir
        end
    end
...
local function get_buf_name(id)
    ...
    if id == nil then
        return get_dir_from_name(vim.api.nvim_buf_get_name(0))
    elseif type(id) == "string" then
        return get_dir_from_name(id)
    end
...
M.add_file = function(file_name_or_buf_id)
    ...
    harpoon.get_finder_config().marks[found_idx] = create_mark(buf_name)
...
M.remove_empty_tail = function(_emit_on_changed)
    ...
    local config = harpoon.get_finder_config()
    local found = false
...
M.store_offset = function()
    ...
    local ok, res = pcall(function()
        ...
        harpoon.get_finder_config().marks[idx].row = cursor_pos[1]
        harpoon.get_finder_config().marks[idx].col = cursor_pos[2]
    end)
...
M.rm_file = function(file_name_or_buf_id)
    ...
    harpoon.get_finder_config().marks[idx] = create_mark("")
    M.remove_empty_tail(false)
    emit_changed()
end

M.clear_all = function()
    harpoon.get_finder_config().marks = {}
...
M.get_marked_file = function(idxOrName)
    ...
    return harpoon.get_finder_config().marks[idxOrName]
end

M.get_marked_file_name = function(idx)
    local mark = harpoon.get_finder_config().marks[idx]
...
M.get_length = function()
    ...
    return table.maxn(harpoon.get_finder_config().marks)
end
