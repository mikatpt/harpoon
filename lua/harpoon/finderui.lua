...
local Marked = require("harpoon.findermark")
...
local function close_menu(force_save)
    ...
    if global_config.save_on_toggle or force_save then
        require("harpoon.finderui").on_menu_save()
    end
...
M.toggle_quick_menu = function()
    ...
    vim.api.nvim_buf_set_keymap(
        ...
        "n",
        "q",
        ":lua require('harpoon.finderui').toggle_quick_menu()<CR>",
        { silent = true }
    )
    vim.api.nvim_buf_set_keymap(
        Harpoon_bufh,
    ...
    vim.api.nvim_buf_set_keymap(
        ...
        "n",
        "<CR>",
        ":lua require('harpoon.finderui').select_menu_item()<CR>",
        {}
    )
    vim.cmd(
        string.format(
            "autocmd BufWriteCmd <buffer=%s> :lua require('harpoon.finderui').on_menu_save()",
            Harpoon_bufh
        )
    )
    if global_config.save_on_change then
        vim.cmd(
            string.format(
                "autocmd TextChanged,TextChangedI <buffer=%s> :lua require('harpoon.finderui').on_menu_save()",
                Harpoon_bufh
            )
        )
    end
    ...
    vim.cmd(
        "autocmd BufLeave <buffer> ++nested ++once :silent lua require('harpoon.finderui').toggle_quick_menu()"
    )
end

-- Technically shouldn't do anything, just trigger search, I guess.
